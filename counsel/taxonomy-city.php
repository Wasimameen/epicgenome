<?php
/**
 * Firms filtered by city.
 *
 * Shares the directory layout: filter rail + card grid, with a city-specific
 * header. Uses the strong empty state for cities with no firms yet.
 *
 * @package Counsel
 */
...
<main id="primary" class="site-main counsel-directory" role="main">
	<div class="counsel-container">
		...
		<header class="counsel-directory__header">
			<p class="counsel-kicker"><?php esc_html_e( 'City', 'counsel' ); ?></p>
			<h1 class="counsel-directory__title">
				<?php
				/* translators: %s: city name */
				printf( esc_html__( 'Lawyers in %s', 'counsel' ), esc_html( single_term_title( '', false ) ) );
				?>
			</h1>
			...
			<?php else : ?>
				<p class="counsel-directory__lede">
					<?php esc_html_e( 'Independent profiles of firms serving this city. Sponsored placements are clearly labeled.', 'counsel' ); ?>
				</p>
			<?php endif; ?>
		</header>
