(function(){
  ...
  if(wm){
    var name="OCEANMARKETING", tld=".COM", html="";
  ...
  form.addEventListener('submit',function(ev){
    ...
    data.append('action','oceanmarketing_offer');
    data.append('nonce', OCEANMARKETING_AJAX.nonce);
    data.append('name', nm);
    ...
    fetch(OCEANMARKETING_AJAX.url,{method:'POST',body:data,credentials:'same-origin'})
      ...
      .then(function(res){
        if(res && res.success){
          ...
          if(d.emailed){
            ...
            show('Thanks \u2014 your offer has been received and recorded. The owner reviews every submission and will reply to '+email+'. You can also reach them directly at contact@oceanmarketing.com.','ok');
          }
          form.reset();
        }else{
          show((res && res.data && res.data.message) || 'Something went wrong sending that. Email contact@oceanmarketing.com directly and it\u2019ll reach me.','err');
        }
      })
      .catch(function(){
        show('Network hiccup. Email contact@oceanmarketing.com directly and it\u2019ll reach me.','err');
      })
